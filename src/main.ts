...
class MermaidIconifySettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .addText((text) =>
        text.setPlaceholder('mdi, lucide, logos…').setValue(this.plugin.settings.searchText).onChange(async (value) => {
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.showOnlyEnabled).onChange(async (value) => {
    ...
    new Setting(containerEl)
      ...
      .addText((text) =>
        text.setPlaceholder('24').setValue(String(this.plugin.settings.cacheTtlHours)).onChange(async (value) => {
    ...
    new Setting(containerEl)
      ...
      .addText((text) =>
        text.setPlaceholder('200').setValue(String(this.plugin.settings.maxIconsInPicker)).onChange(async (value) => {
          const parsed = Number(value);
          if (!Number.isFinite(parsed) || parsed <= 0) return;
    ...
    new Setting(containerEl)
      ...
      .addText((text) =>
        text.setPlaceholder('24').setValue(String(this.plugin.settings.previewSize)).onChange(async (value) => {
          const parsed = Number(value);
    ...
    try {
      ...
      entries.forEach(([prefix, info]) => {
        ...
        setting.addButton((button) =>
          button.setButtonText('Picker').onClick(async () => {
            try {
              await this.plugin.enablePack(prefix);
